...
export function KanbanTask({ id, title, priority, description, onDragStart, onClick }: KanbanTaskProps) {
    const pColor = PRIORITY_STYLES[priority] || { text: "text-slate-500", bg: "bg-slate-100" };
    ...
    return (
        ...
        >
            <span className={`text-[10px] font-bold w-max px-2 py-1 rounded-md uppercase ${pColor.text} ${pColor.bg}`}>
                {priority}
            </span>
