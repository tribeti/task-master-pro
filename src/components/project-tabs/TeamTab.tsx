  ...
  return (
    <div className="flex-1 mt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-20 overflow-y-auto">
        {loading
          ? // Skeleton loading cards
            Array.from({ length: 3 }).map((_, i) => (
              ...
              >
                <div
                  className={`w-24 h-24 rounded-full mb-4 ${isCozy ? "bg-slate-800" : "bg-slate-200"}`}
                ></div>
                <div
                  className={`h-5 w-32 rounded mb-2 ${isCozy ? "bg-slate-800" : "bg-slate-200"}`}
                ></div>
                <div
                  className={`h-3 w-20 rounded mb-6 ${isCozy ? "bg-slate-700" : "bg-slate-100"}`}
                ></div>
                <div
                  className={`h-10 w-full rounded-xl ${isCozy ? "bg-slate-800" : "bg-slate-100"}`}
                ></div>
              </div>
            ))
          : filteredMembers.map((member, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Name & Role */}
                  <h3
                    className={`text-xl font-bold flex items-center gap-2 ${isCozy ? "text-white" : "text-slate-900"}`}
                  >
                    {member.display_name}
                    {member.role === "Owner" && (
                      <span
                  ...
                  <div className="w-full mb-6">
                    <div className="flex justify-between text-xs font-bold">
                      <span
                        className={isCozy ? "text-slate-600" : "text-slate-400"}
                      >
                        Joined
                      </span>
                      <span
                        className={isCozy ? "text-slate-400" : "text-slate-600"}
                      >
        ...
        >
          <div
            className={`w-16 h-16 rounded-full border flex items-center justify-center shadow-sm mb-4 ${
              isCozy
                ? "bg-slate-900 border-slate-800 text-[#FF8B5E]"
      ...
      {memberToRemove && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          ></div>
          <div
            className={`relative rounded-3xl shadow-2xl w-full max-w-sm p-8 mx-4 animate-in fade-in zoom-in-95 duration-200 border ${
              isCozy
                ? "bg-[#0F172A] border-slate-800"
                : "bg-white border-transparent"
            }`}
          >
            ...
            <p
              className={`text-sm mb-6 leading-relaxed ${isCozy ? "text-slate-400" : "text-slate-500"}`}
            >
