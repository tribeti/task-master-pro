  ...
  try {
    ...
    for (const task of tasks) {
      ...
      const existingForTask =
        existingNotifs?.filter((n) => n.task_id === task.id) || [];
      const alreadyNotified = existingForTask.some((n) =>
        n.content?.includes(urgencyStr),
      );

      if (!alreadyNotified) {
        const boardInfo = Array.isArray(task.column)
          ? task.column[0]
          : task.column;
        ...
        if (insertError) {
          console.error(
            `Failed to insert notification for task ${task.id}:`,
            insertError,
          );
        }
      }
