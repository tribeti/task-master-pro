...
export async function POST(request: Request, context: any) {
  ...
  try {
    ...
    if (label.board_id !== column.board_id) {
      return NextResponse.json({ error: "Label does not belong to this board." }, { status: 403 });
    }
    ...
    if (error) return NextResponse.json({ error: "Failed to add label to task." }, { status: 500 });
...
export async function DELETE(request: Request, context: any) {
  ...
  try {
    ...
    if (!labelIdParam) return NextResponse.json({ error: "labelId is required" }, { status: 400 });
    ...
    if (error) return NextResponse.json({ error: "Failed to remove label from task." }, { status: 500 });
    return NextResponse.json({ success: true });
  } catch (error: any) {
    return NextResponse.json({ error: error.message || "Internal server error" }, { status: 500 });
  }
