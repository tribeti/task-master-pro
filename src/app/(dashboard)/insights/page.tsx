...
const PIE_COLORS = ["#34D399", "#28B8FA", "#FF8B5E", "#A78BFA", "#FBBF24", "#F472B6"];
...
export default function InsightsPage() {
        ...
        return (
            <div className="px-10 py-10">
                <div className="bg-red-50 text-red-600 p-6 rounded-xl border border-red-100">
                    <h2 className="font-bold text-lg mb-2">Không thể tải dữ liệu phân tích</h2>
                    <p>{error}</p>
                </div>
            </div>
        );
    ...
    data.forEach((task) => {
        const columnTitle = task.columns?.title || "Unknown";
        const columnTitleLower = columnTitle.toLowerCase();

        // 1. Completed
        if (columnTitleLower.includes("done") || columnTitleLower.includes("hoàn thành")) {
            completedTasks++;
        }

        // 2. Overdue (only if NOT completed)
        if (!columnTitleLower.includes("done") && !columnTitleLower.includes("hoàn thành") && task.deadline) {
        ...
        else if (columnTitleLower.includes("done") || columnTitleLower.includes("hoàn thành")) statusLabel = "Hoàn thành";
            ...
            <div className="px-10 pb-20 flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Tổng số công việc</h3>
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Hoàn thành</h3>
                            <div className="w-10 h-10 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
                            </div>
                        </div>
                        <p className="text-4xl font-black text-slate-800">{completedTasks}</p>
                    </div>

                    {/* Completion Rate Card */}
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Tỷ lệ hoàn thành</h3>
                        ...
                        <div className="flex items-end gap-2">
                            <p className="text-4xl font-black text-slate-800">{completionRate}<span className="text-2xl text-slate-400">%</span></p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 hover:shadow-md hover:-translate-y-1 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Quá hạn</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
                    ...
                    <div className="bg-white rounded-xl p-8 shadow-sm border border-slate-100 hover:shadow-md transition-shadow min-w-0">
                        <h2 className="text-lg font-bold text-slate-900 mb-6">Phân bố Trạng thái Công việc</h2>
                    ...
                    <div className="bg-white rounded-xl p-8 shadow-sm border border-slate-100 hover:shadow-md transition-shadow min-w-0">
                        <h2 className="text-lg font-bold text-slate-900 mb-6">Phân tích Mức độ Ưu tiên</h2>
                        <div className="w-full h-[300px]">
                            {mounted && (
                                <ResponsiveContainer width="100%" height={300} minWidth={0} minHeight={0} debounce={1}>
